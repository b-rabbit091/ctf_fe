        ...
        <>
            ...
            <main className="min-h-screen bg-slate-50 px-4 py-6 md:py-8">
                <div className="mx-auto max-w-6xl">
                    ...
                    {/* Filters */}
                    <section
                        className="mb-4 rounded-xl border border-slate-200 bg-white px-4 py-4 md:px-5 md:py-4 shadow-sm">
                  ...
                  {filteredEntries.length}
                    ...
                    {error && (
                        <div
                            className="mb-4 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 shadow-sm">
                            {error}
                        </div>
                    )}
                    ...
                    {!loading && !error && (
                        <>
                            {filteredEntries.length === 0 ? (
                                <div
                                    className="rounded-xl border border-slate-200 bg-white px-4 py-10 text-center text-slate-500 shadow-sm">
                                    ...
                                    <table className="min-w-full divide-y divide-slate-200 text-sm">
                                        ...
                                        {filteredEntries.map((e) => (
                                            <tr key={`${e.userId ?? e.username}-${e.rank}`}>
                                                <td className="px-4 py-2 align-top text-sm font-semibold text-slate-900">
                                                    #{e.rank}
                                                </td>
                                                ...
                                                <td className="px-4 py-2 align-top text-xs text-slate-500">
                                                    {e.last_submission_at
                                                        ? new Date(e.last_submission_at).toLocaleString()
                                                        : "—"}
                                                </td>
                                            </tr>
                                        ))}
